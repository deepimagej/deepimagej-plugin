...
public class DeepImageJ_Run implements PlugIn, ItemListener {
	...
	public void run(String arg) {
		...
		if (WindowManager.getCurrentImage() == null) {
			...
			if (!version.equals("default (not biozoo format model)") && !version.contains(" (missing") && !version.contains(" (faulty)")) {
			...
			if (!processingFile[0].contains("no preprocessing")) {
				if (dp.params.pre.get(processingFile[0]).length > 0) {
					dp.params.firstPreprocessing = dp.getPath() + File.separator + dp.params.pre.get(processingFile[0])[0];
				}
				if(dp.params.pre.get(processingFile[0]).length > 1) {
					dp.params.secondPreprocessing = dp.getPath() + File.separator + dp.params.pre.get(processingFile[0])[1];
				}
			}
			if (!processingFile[1].contains("no preprocessing")) {
				if (dp.params.post.get(processingFile[1]).length > 0) {
					dp.params.firstPostprocessing = dp.getPath() + File.separator + dp.params.post.get(processingFile[1])[0];
				}
				if(dp.params.post.get(processingFile[1]).length > 1) {
	...
	public void itemStateChanged(ItemEvent e) {
		if (e.getSource() == choices[0]) {
			...
			if (versions.get("correct").size() == 0) {
				choices[1].removeAll();
				choices[1].addItem("default (not biozoo format model)");
	...
	public void calculateImage(ImagePlus inp) {
		...
		try {
			...
			List<String> fAList = Arrays.asList(finalFrames);
			List<String> iAList = Arrays.asList(finalImages);
			for (String outs : output.keySet()) {
				Object f = output.get(outs);
				if (f != null && f instanceof TextWindow) {
		        	ResultsTable table = ((TextWindow)f).getResultsTable();
					String title = table.getTitle();
					// TODO check some random values to make sure it is not the same image
					if (fAList.contains(title)) {
						String newTitle = title;
						int c = 1;
						while (fAList.contains(newTitle)) {
							newTitle = title + "-" + c;
							c ++;
						}
						title = newTitle;
					...
					// TODO check some random values to make sure it is not the same image
					if (iAList.contains(title)) {
						String newTitle = title;
						int c = 1;
						while (iAList.contains(newTitle)) {
							newTitle = title + "-" + c;
							c ++;
						}
