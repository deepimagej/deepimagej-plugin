...
public class SigmoidBce extends AbstractLoss {
		...
		for (int z=1; z<=Math.max(nzr, nzt); z++) {
			...
			for (int x = 0; x < nxr; x++) {
				for (int y = 0; y < nyr; y++) {
					...
					if (!Double.isNaN(g))
						if (!Double.isNaN(s)) {
							bce+=g-(g*s)+Math.log(1+Math.exp(-Math.abs(g)));
						}
				}
			}
			res.add(bce/(nxr*nyr));
	...
	public String check(ImagePlus reference, ImagePlus test, Constants setting) {
		...
		if((max_im1 > 1.0)||(min_im1<0.0)) {
			return "For Bce, reference values must be between 0.0 and 1.0" ;
		}
		return "Valid";
	}
